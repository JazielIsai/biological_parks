...
export const EditPark = ({route, navigation}) => {
    
    const {user} = useContext(AuthContext);

    const {idPark} = route.params;
    console.log('param', idPark);
    ...
    const { data : getParkData } = useFetchGet(`get_park_by_id&id=${idPark}`)
    ...
    useEffect ( ( ) => {
        ...
        try {
            ...
            const dataPark = JSON.parse(getParkData)[0];

            onReset({
                id: dataPark.id,
                namePark: dataPark.namePark,
                trainingbackground: dataPark.trainingBackground,
                areaha: dataPark.areaHa,
                form: dataPark.form,
                boundaries: dataPark.boundaries,
                recreationareas: dataPark.recreationAreas,
                street: dataPark.street,
                suburb: dataPark.suburb,
                latitude: `${dataPark.latitude}`,
                length: `${dataPark.length}`,
                cityStateId: dataPark.idCityStates,
                nameMunicipalityId: dataPark.idMunicipality,
            })
    ...
    }, [getAllCityState, getAllMunicipalyByCityStateId, getParkData] )

    const sendPostToUpdate = () => {
        ...
        requestPost('update_park_data', formData)
            .then( res => {
                console.log(res);
                if (res.includes('1')) {
                    Alert.alert("Actualización exitosa", "Tus datos han sido modificados correctamente");
                } else {
                    Alert.alert("Error", "No se pudo actualizar tus datos");
                }
            } )
    }
